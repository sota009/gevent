...
preamble-tier: 2
version: 0.1.0
description: Personalize an attendee-facing Adaptive Live Deck from deck.json, a preset, and a natural-language attendee request. Produces preference JSON and rendering (gstack)
allowed-tools:
  - Read
  - Write
  - Edit
...
guidance for attendee.html while preserving the speaker-authored
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"adaptive-attendee-deck","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"adaptive-attendee-deck","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstack/bin/gstack-learnings-log '{"skill":"adaptive-attendee-deck","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
